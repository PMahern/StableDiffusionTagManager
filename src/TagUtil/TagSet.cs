        ...
        public TagSet(string file)
        {
            ...
            if (System.IO.File.Exists(file))
            {
                ...
                if (content != null)
                {
                    ...
                    // If there's more than one comma per 25 characters and no periods (sentences end with periods; tags don't) assume tags.
                    if (extractedTags.Count > (lastChunk.Length / 25) && !lastChunk.Contains('.'))
                    {
                        Tags = extractedTags;
                    }
                    else
